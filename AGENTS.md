...
Note: there is backward compatibility policy, breaking changes are allowed.
...
| `PDFA1B-FONT-EMBEDDING-001` | `ISO 19005-1:2005:6.3.4:1` | §6.3.4 | partial/proxy | Implements `Subtype == "Type3" \|\| Subtype == "Type0" \|\| renderingMode == 3 \|\| containsFontFile == true` for bounded page/Form text-show paths and used Type0 descendants. |
| `PDFA1B-ICCBASED-001` | `ISO 19005-1:2005:6.2.3.2:1` | §6.2.3.2 | partial/proxy | For safely decoded profiles reached by the documented bounded content paths: `(deviceClass == "prtr" \|\| deviceClass == "mntr" \|\| deviceClass == "scnr" \|\| deviceClass == "spac") && (colorSpace == "RGB " \|\| colorSpace == "CMYK" \|\| colorSpace == "GRAY" \|\| colorSpace == "Lab ") && version < 3.0`. |
| `PDFA1B-ICCBASED-COMPONENTS-001` | `ISO 19005-1:2005:6.2.3.2:2` | §6.2.3.2 | partial/proxy | For safely decoded profiles reached by the documented bounded content paths: `N != null && ((N == 1 && colorSpace == "GRAY") \|\| (N == 3 && (colorSpace == "RGB " \|\| colorSpace == "Lab ")) \|\| (N == 4 && colorSpace == "CMYK"))`. |
| `PDFA1B-DEVICE-RGB-001` | `ISO 19005-1:2005:6.2.3.3:1` | §6.2.3.3 | partial/proxy | For DeviceRGB uses reached by the documented bounded content paths: `gOutputCS != null && gOutputCS == "RGB "`. |
| `PDFA1B-DEVICE-CMYK-001` | `ISO 19005-1:2005:6.2.3.3:2` | §6.2.3.3 | partial/proxy | For DeviceCMYK uses reached by the documented bounded content paths: `gOutputCS != null && gOutputCS == "CMYK"`. |
| `PDFA1B-DEVICE-GRAY-001` | `ISO 19005-1:2005:6.2.3.3:3` | §6.2.3.3 | partial/proxy | For DeviceGray uses reached by the documented bounded content paths: `gOutputCS != null`. |
| `PDFA1B-IMAGE-ALTERNATES-001` | `ISO 19005-1:2005:6.2.4:1` | §6.2.4 | partial/proxy | For invoked Image XObjects: `containsAlternates == false`. |
| `PDFA1B-XOBJECT-OPI-001` | `ISO 19005-1:2005:6.2.4:2` | §6.2.4 | partial/proxy | For invoked XObjects: `containsOPI == false`. |
| `PDFA1B-IMAGE-INTERPOLATE-001` | `ISO 19005-1:2005:6.2.4:3` | §6.2.4 | partial/proxy | For invoked Image XObjects: `Interpolate == false`. |
| `PDFA1B-IMAGE-BPC-001` | `ISO 19005-1:2005:6.2.4:4` | §6.2.4 | partial/proxy | For invoked Image XObjects: `isMask == true \|\| BitsPerComponent == null \|\| BitsPerComponent == 1 \|\| BitsPerComponent == 2 \|\| BitsPerComponent == 4 \|\| BitsPerComponent == 8`. |
| `PDFA1B-IMAGE-MASK-BPC-001` | `ISO 19005-1:2005:6.2.4:5` | §6.2.4 | partial/proxy | For images reached through another image's `/Mask` reference: `BitsPerComponent == null \|\| BitsPerComponent == 1`. |
| `PDFA1B-FORM-POSTSCRIPT-001` | `ISO 19005-1:2005:6.2.5:1` | §6.2.5 | partial/proxy | For invoked Form XObjects: `(Subtype2 == null \|\| Subtype2 != "PS") && containsPS == false`. |
| `PDFA1B-FORM-REFERENCE-001` | `ISO 19005-1:2005:6.2.6:1` | §6.2.6 | partial/proxy | For invoked Form XObjects: `containsRef == false`. |
| `PDFA1B-XOBJECT-POSTSCRIPT-001` | `ISO 19005-1:2005:6.2.7:1` | §6.2.7 | partial/proxy | For invoked XObjects: `Subtype != "PS"`. |
| `PDFA1B-EXTGSTATE-TR-001` | `ISO 19005-1:2005:6.2.8:1` | §6.2.8 | partial/proxy | For indirect ExtGState dictionaries selected by executed `gs` operators: `containsTR == false`. |
| `PDFA1B-EXTGSTATE-TR2-001` | `ISO 19005-1:2005:6.2.8:2` | §6.2.8 | partial/proxy | For indirect ExtGState dictionaries selected by executed `gs` operators: `containsTR2 == false \|\| TR2NameValue == "Default"`. |
| `PDFA1B-RENDERING-INTENT-001` | `ISO 19005-1:2005:6.2.9:1` | §6.2.9 | partial/proxy | For rendering intents reached through executed `ri` and `gs` operators or invoked Image XObjects: the name is `RelativeColorimetric`, `AbsoluteColorimetric`, `Perceptual`, or `Saturation`. |
| `PDFA1B-CONTENT-OPERATOR-001` | `ISO 19005-1:2005:6.2.10:1` | §6.2.10 | partial/proxy | No undefined operator occurs in bounded executed page or Form content, including compatibility sections delimited by `BX` and `EX`. |
| `PDFA1B-FONT-TYPE-001` | `ISO 19005-1:2005:6.3.2:1` | §6.3.2 | partial/proxy | For fonts reached by bounded text-show paths: `Type == "Font"`. |
| `PDFA1B-FONT-SUBTYPE-001` | `ISO 19005-1:2005:6.3.2:2` | §6.3.2 | partial/proxy | Modeled fonts have subtype `Type1`, `MMType1`, `TrueType`, `Type3`, `Type0`, `CIDFontType0`, or `CIDFontType2`; missing/unsupported subtypes create no veraPDF `PDFont` object and are inapplicable. |
| `PDFA1B-FONT-BASEFONT-001` | `ISO 19005-1:2005:6.3.2:3` | §6.3.2 | partial/proxy | For fonts reached by bounded text-show paths: `Subtype == "Type3" \|\| fontName != null`. |
| `PDFA1B-FONT-FIRSTCHAR-001` | `ISO 19005-1:2005:6.3.2:4` | §6.3.2 | partial/proxy | For modeled simple fonts: `isStandard == true \|\| FirstChar != null`. |
| `PDFA1B-FONT-LASTCHAR-001` | `ISO 19005-1:2005:6.3.2:5` | §6.3.2 | partial/proxy | For modeled simple fonts: `isStandard == true \|\| LastChar != null`. |
| `PDFA1B-FONT-WIDTHS-001` | `ISO 19005-1:2005:6.3.2:6` | §6.3.2 | partial/proxy | For modeled simple fonts: `isStandard == true \|\| (Widths_size != null && Widths_size == LastChar - FirstChar + 1)`. |
| `PDFA1B-FONT-FILE-SUBTYPE-001` | `ISO 19005-1:2005:6.3.2:7` | §6.3.2 | partial/proxy | For embedded font streams reached through modeled fonts: `fontFileSubtype == null \|\| fontFileSubtype == "Type1C" \|\| fontFileSubtype == "CIDFontType0C"`. |
| `PDFA1B-TYPE0-CID-SYSTEM-INFO-001` | `ISO 19005-1:2005:6.3.3.1:1` | §6.3.3.1 | partial/proxy | `cmapName == "Identity-H" \|\| cmapName == "Identity-V" \|\| (CIDFontOrdering != null && CIDFontOrdering == CMapOrdering && CIDFontRegistry != null && CIDFontRegistry == CMapRegistry)` over embedded CMaps reached by bounded text-show paths. |
| `PDFA1B-CIDTOGIDMAP-001` | `ISO 19005-1:2005:6.3.3.2:1` | §6.3.3.2 | partial/proxy | For used first descendants: `Subtype != "CIDFontType2" \|\| CIDToGIDMap != null \|\| renderingMode == 3`; a modeled map is `Identity` or a stream. |
| `PDFA1B-CMAP-EMBEDDING-001` | `ISO 19005-1:2005:6.3.3.3:1` | §6.3.3.3 | partial/proxy | For Type0 fonts reached by bounded text-show paths: `CMapName == "Identity-H" \|\| CMapName == "Identity-V" \|\| containsEmbeddedFile == true`. |
...
| `PDFA1B-TRUETYPE-GLYPH-PRESENCE-001` | `ISO 19005-1:2005:6.3.5:1` | §6.3.5 | partial/proxy | For TrueType, `CIDFontType2`, and CID-keyed CFF (`CIDFontType0C`) fonts reached by bounded text-show paths: a bounded embedded program defines a glyph for every rendered byte or CID. |
...
| `PDFA1B-TRUETYPE-SYMBOLIC-CMAP-001` | `ISO 19005-1:2005:6.3.7:3` | §6.3.7 | partial/proxy | For recognized embedded symbolic TrueType programs: `nrCmaps == 1`, read from the bounded SFNT `cmap` table header. |
...
| `PDFA1B-FILE-SPEC-EMBEDDED-FILE-001` | `ISO 19005-1:2005:6.1.11:1` | §6.1.11 | exact | For dictionary-based values in the catalog's `EmbeddedFiles` name tree: `containsEF == false`; `Names` arrays and recursive `Kids` are traversed with configured reference-depth bounds. |
...
| `PDFA1B-INLINE-IMAGE-LZW-001` | `ISO 19005-1:2005:6.1.10:2` | §6.1.10 | partial/proxy | No inline image in bounded executed page/Form content declares `internalRepresentation == "LZWDecode"` (or the inline-image-only `LZW` spelling). |
...
| `PDFA1B-DEVICEN-COMPONENTS-001` | `ISO 19005-1:2005:6.1.12:9` | §6.1.12 | partial/proxy | Every DeviceN colour space array reached anywhere in the parsed object graph, or through the bounded content-execution paths, declares at most 8 components. |
