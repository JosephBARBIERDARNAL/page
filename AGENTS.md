...
| `PDFA1B-TYPE1-GLYPH-PRESENCE-001` | `ISO 19005-1:2005:6.3.5:1` | §6.3.5 | partial/proxy | For simple Type1, MMType1, and Type1C fonts reached by bounded text-show paths: a bounded embedded program defines a glyph for every rendered byte. Still `partial/proxy`: shares the `type1_standard_glyph_name`-fallback gap documented under `TYPE1-SUBSET-CHARSET-001` (a byte absent from `/Differences` always resolves via a hardcoded StandardEncoding table, never the embedded Type1 program's own built-in `/Encoding` array). The whole hand-rolled `type1_program` fixture family (every `type1_glyph_*`/`type1_difference_glyph`/`type1_width_mismatch` case) is confirmed to crash veraPDF 1.28.2 itself (`Exception: ... Range [0, 0 + -3) out of bounds for length 8192`, reproduced directly) when dumped for live differential comparison, since these fixtures were only ever built to exercise this crate's own byte-level Type1 scanning (`type1_program_char_names`/`type1_program_charstring_widths`), not to be a spec-complete Type1 program a real Type1 parser can execute -- so none of this family's local behavior has ever been confirmed against live veraPDF; `atomic_font_cases` has zero entries for it. |
| `PDFA1B-TRUETYPE-GLYPH-PRESENCE-001` | `ISO 19005-1:2005:6.3.5:1` | §6.3.5 | partial/proxy | For TrueType, `CIDFontType2`, and CID-keyed CFF (`CIDFontType0C`) fonts reached by bounded text-show paths: a bounded embedded program defines a glyph for every rendered byte or CID. For `CIDFontType2`, `/CIDToGIDMap` is resolved through indirection before the `/Identity` name check (confirmed live via the paired `PDFA1B-TRUETYPE-GLYPH-WIDTH-001` fix); an unresolvable map is treated as inapplicable (silently skipped) rather than a missing-glyph failure, since `PDFA1B-CIDTOGIDMAP-001` already flags a genuinely missing/invalid map separately. Still `partial/proxy`: glyph lookup uses `ttf_parser::Face::parse` (requiring `head`/`hhea`/`maxp`/`hmtx`/`loca`/`glyf` to all parse), unlike `PDFA1B-FONT-EMBEDDING-001`/`PDFA1B-TRUETYPE-SYMBOLIC-CMAP-001`'s narrower `RawFace` reads; confirmed live that veraPDF still evaluates this predicate (and flags a missing glyph) for a font whose `cmap`/`FontFile2` signature is otherwise valid but whose `maxp` is malformed enough that `Face::parse` fails, which the local implementation instead silently skips. No safe `ttf_parser` API path to replicate this narrower behavior was found (glyph/`loca` lookup requires `numberOfGlyphs`/`indexToLocFormat`, which come from the same `maxp`/`head` tables that are malformed in the confirmed counter-example). |
| `PDFA1B-TYPE1-SUBSET-CHARSET-001` | `ISO 19005-1:2005:6.3.5:2` | §6.3.5 | partial/proxy | For a subset Type1/MMType1 font (`BaseFont` with a six-uppercase-letter subset tag): the descriptor `/CharSet` string names every rendered glyph. `/BaseFont` is resolved through indirection before the subset-tag check (confirmed live: an indirect reference to a subset-tagged name is recognized exactly like a direct one, so an incomplete `/CharSet` is still caught rather than the font being treated as non-subset and the check silently skipped). Still `partial/proxy`: a rendered byte's glyph name, when absent from `/Differences`, always falls back to a hardcoded StandardEncoding table (`type1_standard_glyph_name`) rather than reading the embedded Type1 program's own built-in `/Encoding` array (PostScript-level, distinct from the PDF `/Encoding` dictionary) -- a font relying on a non-standard built-in encoding (e.g. Symbol-like fonts) would resolve the wrong glyph name. Not independently live-confirmed: the whole `type1_program`-based fixture family shares the veraPDF 1.28.2 crash documented under `TYPE1-GLYPH-PRESENCE-001` below, blocking further live testing of this family. |
| `PDFA1B-CID-SUBSET-CIDSET-001` | `ISO 19005-1:2005:6.3.5:3` | §6.3.5 | partial/proxy | For a subset `CIDFontType0`/`CIDFontType2` descendant: the descriptor `/CIDSet` stream's bits identify every rendered CID as present. The descendant's own `/Subtype` and `/BaseFont` are both resolved through indirection before the subset-tag check (confirmed live: an indirect reference to a subset-tagged `/BaseFont` is recognized exactly like a direct one, so a `/CIDSet` omitting a rendered CID is still caught rather than the descendant being treated as non-subset and the check silently skipped). Still `partial/proxy` for the same reason as `TYPE0-CID-SYSTEM-INFO-001`: this predicate first needs `cached_cids_for_rendered_bytes` to decode the rendered bytes into CIDs, which silently yields no CIDs (and so silently skips this whole check) when `/Encoding` is a predefined, non-Identity, non-embedded named CMap, since that collection is not modeled locally. |
| `PDFA1B-TRUETYPE-GLYPH-WIDTH-001` | `ISO 19005-1:2005:6.3.6:1` | §6.3.6 | partial/proxy | For TrueType, `CIDFontType2`, Type1, and Type1C/`CIDFontType0C` fonts reached by bounded text-show paths: a rendered byte or CID's width in the embedded program agrees with its dictionary-declared width within 1 unit, matching veraPDF's `abs(...) <= 1` tolerance. For `CIDFontType2`, `/CIDToGIDMap` is resolved through indirection before the `/Identity` name check (confirmed live: an indirect `/Identity` reference resolves rendered CIDs to the same glyphs as a direct one, so a genuine width mismatch is still caught rather than silently skipped). For simple fonts, both the whole `/Widths` array and each individual entry it reads are resolved through indirection before comparison (confirmed live for an indirect array and, separately, for a direct array whose one entry is an indirect reference to a mismatched value -- the mismatch is still caught rather than silently skipped). A descendant's own `/Subtype` (the `CIDFontType0`/`CIDFontType2` selector) is resolved through indirection before this and the paired presence check run at all (confirmed live: an indirect reference to `/CIDFontType2` is recognized exactly like a direct one, so a genuine width mismatch is still caught rather than the descendant being treated as an unrecognized subtype and silently skipped). For CID fonts, `/DW` and each `/W` singles-group width entry are likewise resolved through indirection before comparison (confirmed live for both). Still `partial/proxy` for the same `ttf_parser::Face::parse`-vs-`RawFace` reason as `TRUETYPE-GLYPH-PRESENCE-001`: a font whose `maxp` is malformed enough to fail `Face::parse` is still width-checked by veraPDF but silently skipped locally. |
