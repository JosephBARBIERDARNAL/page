...
| `PDFA1B-FONT-SUBTYPE-001` | `ISO 19005-1:2005:6.3.2:2` | §6.3.2 | exact | Modeled fonts have subtype `Type1`, `MMType1`, `TrueType`, `Type3`, `Type0`, `CIDFontType0`, or `CIDFontType2`; missing/unsupported subtypes create no veraPDF `PDFont` object and are inapplicable, so this predicate cannot fail on either side by construction (confirmed live: a font with an unsupported `/Subtype`, used normally, is fully compliant on both). |
...
| `PDFA1B-FONT-WIDTHS-001` | `ISO 19005-1:2005:6.3.2:6` | §6.3.2 | exact | For modeled simple fonts: `isStandard == true \|\| (Widths_size != null && Widths_size == LastChar - FirstChar + 1)`. `isStandard` is confirmed live to apply only to `Type1`/`MMType1`, not `TrueType`/`Type3`. |
