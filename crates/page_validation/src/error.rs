...
///
/// Each variant is either a strict-parser rejection (for example a malformed cross-reference table) or one of the configurable [`SafetyLimits`](crate::SafetyLimits) bounds being exceeded, such as an oversized input or an over-deep reference chain. [`ValidationError::Pdf`] wraps this type for the public [`validate_bytes`](crate::validate_bytes) and [`validate_file`](crate::validate_file) entry points, so callers that only need the top-level outcome can match on [`ValidationError`] instead.
///
/// ## Examples
///
/// ```
...
/// The top-level error returned by [`validate_bytes`](crate::validate_bytes) and [`validate_file`](crate::validate_file) when a document cannot be scored against a profile at all.
///
/// This is distinct from a [`ValidationReport`](crate::ValidationReport) recording failures: a report means the profile's rules ran and found conformance problems, while `ValidationError` means the input could not be read, parsed, or matched to a profile in the first place. [`Self::Pdf`] carries the lower-level [`PdfError`] from parsing or inspecting the object graph.
///
/// ## Examples
///
/// ```
