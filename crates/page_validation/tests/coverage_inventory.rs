...
const RULE_MAPPING_DOC_PATH: &str = "../../docs/rules/pdfa-1-rule-mapping.md";
...
fn generated_rule_mapping_documentation(inventory: &Value) -> String {
    let mut output = String::from(
        "---\n\
         title: PDF/A-1 pinned rule mapping\n\
         ---\n\n\
    ...
    for mapping in array(
        &inventory["pdfa_1a_only_mappings"],
        "PDF/A-1a-only mappings",
    ) {
        let local_rule = string(&mapping["local_rule_id"], "PDF/A-1a local rule id");
        let reference_rule = string(&mapping["verapdf_rule_id"], "PDF/A-1a veraPDF rule id");
        let clause = reference_rule
            .strip_prefix("ISO 19005-1:2005:")
            .and_then(|suffix| suffix.rsplit_once(':'))
            .map(|(clause, _)| clause)
            .expect("PDF/A-1a veraPDF rule id");
        let strength = string(
            &mapping["implementation_strength"],
            "PDF/A-1a implementation strength",
        );
        let note = markdown_table_text(string(&mapping["mapping_note"], "PDF/A-1a mapping note"));
        output.push_str(&format!(
            "| `{local_rule}` | `{reference_rule}` | §{clause} | {strength} | {note} |\n"
        ));
    }
