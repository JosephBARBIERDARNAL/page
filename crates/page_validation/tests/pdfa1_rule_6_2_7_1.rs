...
const RULE: &str = "PDFA1B-XOBJECT-POSTSCRIPT-001";
const ADDITIONAL_RULES: &[&str] = &[];
const REFERENCE_RULE: &str = "ISO 19005-1:2005:6.2.7:1";
...
#[test]
fn pdfa1_rule_6_2_7_1_local_validation() {
...
#[test]
#[ignore = "maintenance generator for PDFA1 rule 6.2.7-1 fixtures"]
fn pdfa1_rule_6_2_7_1_fixture_generation() {
    let profile = PROFILES[0];
    let bytes = common::pdfa_profile_fixture(profile, common::canonical_pdfa_fixture(profile));
    common::write_generated_fixture("pdfa1-rule-6-2-7-1-valid.pdf", &bytes);
    let mut local_rules = vec![RULE];
    local_rules.extend_from_slice(ADDITIONAL_RULES);
    for local_rule in &local_rules {
        if let Some(source) = common::mutation_fixture(local_rule) {
            common::write_generated_fixture("pdfa1-rule-6-2-7-1-invalid.pdf", &source);
            break;
        }
    }
}
...
#[test]
fn pdfa1_rule_6_2_7_1_verapdf_differential_when_opted_in() {
    let mut local_rules = vec![RULE];
    local_rules.extend_from_slice(ADDITIONAL_RULES);
