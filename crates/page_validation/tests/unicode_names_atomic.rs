...
type Fixture = fn(&str) -> Vec<u8>;
type UnicodeNameCase = (&'static str, Fixture, bool);

const CASES: &[UnicodeNameCase] = &[
            ...
            assert_eq!(
                validate_bytes_with_profile(&bytes, profile, &SafetyLimits::default())
                    ...
                    .any(|failure| failure.rule_id == "PDFA2B-UNICODE-NAME-001"
                        || failure.rule_id == "PDFA3B-UNICODE-NAME-001"),
                *expected_failure,
