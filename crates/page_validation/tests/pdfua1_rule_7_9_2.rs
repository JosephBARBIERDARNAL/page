use std::collections::BTreeSet;
use std::env;
use std::fs;
use std::path::Path;

use page_validation::differential::{DifferentialRunner, ReferenceConfig, ReferenceProfile};
use page_validation::{SafetyLimits, ValidationProfile, validate_bytes_with_profile};

pub mod common;

const RULE: &str = "PDFUA1-NOTE-ID-UNIQUE-001";
const REFERENCE_RULE: &str = "ISO 14289-1:2014:7.9:2";

#[test]
fn pdfua1_rule_7_9_2_requires_unique_note_ids() {
    let unique = validate_bytes_with_profile(
        include_bytes!("fixtures/pdfua1-rule-7-9-2-unique.pdf"),
        ValidationProfile::PdfUa1,
        &SafetyLimits::default(),
    );
    assert!(unique.checks_passed, "{unique}");
    assert!(unique.failures.is_empty());

    let duplicate = validate_bytes_with_profile(
        include_bytes!("fixtures/pdfua1-rule-7-9-2-duplicate.pdf"),
        ValidationProfile::PdfUa1,
        &SafetyLimits::default(),
    );
    assert!(!duplicate.checks_passed, "{duplicate}");
    assert_eq!(duplicate.checks.failed, 1, "{duplicate}");
    assert_eq!(duplicate.failures.len(), 1, "{duplicate}");
    assert_eq!(duplicate.failures[0].rule_id, RULE, "{duplicate}");
}

#[test]
#[ignore = "maintenance generator for PDF/UA-1 rule 7.9-2 fixtures"]
fn regenerate_pdfua1_rule_7_9_2_fixtures() {
    for (fixture, case) in [
        ("pdfua1-rule-7-9-2-unique.pdf", "unique"),
        ("pdfua1-rule-7-9-2-duplicate.pdf", "duplicate"),
    ] {
        fs::write(
            Path::new("tests/fixtures").join(fixture),
            common::pdfua1_rule_7_9_2_fixture(case),
        )
        .expect("write PDF/UA-1 rule 7.9-2 fixture");
    }
}

#[test]
fn pdfua1_rule_7_9_2_fixtures_match_verapdf_1302_when_opted_in() {
    let Some(executable) = env::var_os("VERAPDF_BIN") else {
        return;
    };
    let mut config = ReferenceConfig::pinned(executable);
    config.profile = ReferenceProfile::PdfUa1;
    let runner = DifferentialRunner::new(config).expect("pinned veraPDF 1.30.2");
    for (fixture, should_fail) in [
        ("pdfua1-rule-7-9-2-unique.pdf", false),
        ("pdfua1-rule-7-9-2-duplicate.pdf", true),
    ] {
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures")
            .join(fixture);
        let report = runner.compare_file(&path, &SafetyLimits::default());
        let failed = report
            .reference_result
            .as_ref()
            .expect("veraPDF result")
            .failed_rule_ids
            .iter()
            .map(ToString::to_string)
            .collect::<BTreeSet<_>>();
        assert_eq!(
            failed.contains(REFERENCE_RULE),
            should_fail,
            "{fixture}: {report}"
        );
    }
}
