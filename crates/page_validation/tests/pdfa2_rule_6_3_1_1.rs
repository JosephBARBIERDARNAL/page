...
const RULE: &str = "PDFA1B-ANNOTATION-SUBTYPE-001";
const ADDITIONAL_RULES: &[&str] = &[];
const REFERENCE_RULE: &str = "ISO 19005-2:2011:6.3.1:1";
...
#[test]
fn pdfa2_rule_6_3_1_1_local_validation() {
...
#[test]
#[ignore = "maintenance generator for PDFA2 rule 6.3.1-1 fixtures"]
fn pdfa2_rule_6_3_1_1_fixture_generation() {
    let profile = PROFILES[0];
    let bytes = common::pdfa_profile_fixture(profile, common::canonical_pdfa_fixture(profile));
    common::write_generated_fixture("pdfa2-rule-6-3-1-1-valid.pdf", &bytes);
    let mut local_rules = vec![RULE];
    local_rules.extend_from_slice(ADDITIONAL_RULES);
    for local_rule in &local_rules {
        if let Some(source) = common::mutation_fixture(local_rule) {
            common::write_generated_fixture("pdfa2-rule-6-3-1-1-invalid.pdf", &source);
            break;
        }
    }
}
...
#[test]
fn pdfa2_rule_6_3_1_1_verapdf_differential_when_opted_in() {
    let mut local_rules = vec![RULE];
    local_rules.extend_from_slice(ADDITIONAL_RULES);
