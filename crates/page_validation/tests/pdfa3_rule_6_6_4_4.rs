...
const REFERENCE_RULE: &str = "ISO 19005-3:2012:6.6.4:4";
...
#[test]
fn pdfa3_rule_6_6_4_4_local_validation() {
...
#[test]
#[ignore = "maintenance generator for PDFA3 rule 6.6.4-4 fixtures"]
fn pdfa3_rule_6_6_4_4_fixture_generation() {
    let profile = PROFILES[0];
    let bytes = common::pdfa_profile_fixture(profile, common::canonical_pdfa_fixture(profile));
    common::write_generated_fixture("pdfa3-rule-6-6-4-4-valid.pdf", &bytes);
    let mut local_rules = vec![RULE];
    local_rules.extend_from_slice(ADDITIONAL_RULES);
    for local_rule in &local_rules {
        if let Some(source) = common::mutation_fixture(local_rule) {
            common::write_generated_fixture("pdfa3-rule-6-6-4-4-invalid.pdf", &source);
            break;
        }
    }
}
...
#[test]
fn pdfa3_rule_6_6_4_4_verapdf_differential_when_opted_in() {
    let mut local_rules = vec![RULE];
    local_rules.extend_from_slice(ADDITIONAL_RULES);
