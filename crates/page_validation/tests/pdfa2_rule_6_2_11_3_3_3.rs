...
const REFERENCE_RULE: &str = "ISO 19005-2:2011:6.2.11.3.3:3";
...
#[test]
fn pdfa2_rule_6_2_11_3_3_3_local_validation() {
...
#[test]
#[ignore = "maintenance generator for PDFA2 rule 6.2.11.3.3-3 fixtures"]
fn pdfa2_rule_6_2_11_3_3_3_fixture_generation() {
    let profile = PROFILES[0];
    let bytes = common::pdfa_profile_fixture(profile, common::canonical_pdfa_fixture(profile));
    common::write_generated_fixture("pdfa2-rule-6-2-11-3-3-3-valid.pdf", &bytes);
    let mut local_rules = vec![RULE];
    local_rules.extend_from_slice(ADDITIONAL_RULES);
    for local_rule in &local_rules {
        if let Some(source) = common::mutation_fixture(local_rule) {
            common::write_generated_fixture("pdfa2-rule-6-2-11-3-3-3-invalid.pdf", &source);
            break;
        }
    }
}
...
#[test]
fn pdfa2_rule_6_2_11_3_3_3_verapdf_differential_when_opted_in() {
    let mut local_rules = vec![RULE];
    local_rules.extend_from_slice(ADDITIONAL_RULES);
