...
const RULE: &str = "PDFA1B-STREAM-EOL-001";
const ADDITIONAL_RULES: &[&str] = &[];
const REFERENCE_RULE: &str = "ISO 19005-3:2012:6.1.7.1:2";
...
#[test]
fn pdfa3_rule_6_1_7_1_2_local_validation() {
...
#[test]
#[ignore = "maintenance generator for PDFA3 rule 6.1.7.1-2 fixtures"]
fn pdfa3_rule_6_1_7_1_2_fixture_generation() {
    let profile = PROFILES[0];
    let bytes = common::pdfa_profile_fixture(profile, common::canonical_pdfa_fixture(profile));
    common::write_generated_fixture("pdfa3-rule-6-1-7-1-2-valid.pdf", &bytes);
    let mut local_rules = vec![RULE];
    local_rules.extend_from_slice(ADDITIONAL_RULES);
    for local_rule in &local_rules {
        if let Some(source) = common::mutation_fixture(local_rule) {
            common::write_generated_fixture("pdfa3-rule-6-1-7-1-2-invalid.pdf", &source);
            break;
        }
    }
}
...
#[test]
fn pdfa3_rule_6_1_7_1_2_verapdf_differential_when_opted_in() {
    let mut local_rules = vec![RULE];
    local_rules.extend_from_slice(ADDITIONAL_RULES);
