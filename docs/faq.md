...
In the long run, I hope that `page` will provide a veraPDF-compliant alternative, that is faster and very lightweight. [Latest measurements](./#performance) suggest that `page` could be around an order of magnitude faster and use correspondingly less memory (peak RSS). Reasons for those difference is thatc`page` is written in Rust and `veraPDF` in Java.
